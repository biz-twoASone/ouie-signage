...
**Status (as of 2026-04-22):** **Plan 1 complete. Plan 2 mid-execution.** Plan 2 written as `docs/superpowers/plans/2026-04-21-plan-2-dashboard.md` and executing on branch `feature/plan-2-dashboard`. **Batches A, B, C, D done.** Playwright E2E verified end-to-end: magic-link login, tenant auto-provisioning, stores CRUD, pair-TV flow, devices list/detail + Sync Now (202 from FCM), R2 two-phase upload + delete (row + object both cleaned up via new `media-delete` Edge Function and `aws4fetch`-powered `r2Delete` in Next), playlist composer with add/remove items, per-device fallback playlist, store batch playlist assignment ("1 devices / Main loop" common detection works), device groups with member toggle. Drag-reorder not exercised in Playwright (single-item list) — two-phase SQL reorder is unit-correct; revisit in Task 22 Playwright suite. Remote Supabase project `signage-ouie` (ref `swhwrlpoqjijxcvywzto`) has all 16 Plan 1 migrations + the 8 Plan 1 Edge Functions. Plan 2 additions that are **local-only** (push when Vercel URL is finalized in Batch F): migration `20260422000050_media_pending_support`, migration `20260422000100_tenant_bootstrap_trigger` (already pushed), Edge Functions `media-upload-url` and `media-delete`. R2 bucket CORS for localhost set with `AllowedHeaders: ["Content-Type"]`. Remote `site_url` / `additional_redirect_urls` still local-only. Known cosmetic nit: Store edit form shows `02:00:00` in time inputs (list page is fine at `02:00`); trivial slice fix if you want it. Next: Batch E (Tasks 18–21, dayparting rules + pg_cron offline-device alerts via Resend + heartbeat schema polish — requires Resend API key).
